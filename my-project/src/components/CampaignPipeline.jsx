// src/pages/CampaignPipeline.jsx
...
function CampaignPipelineInternal({ campaignId, onRefresh }) {
  ...
  useEffect(() => {
    ...
    if (pipelineData.bookingStatus?.confirmed) {
      staticNodes.push(
        { id: 'po', data: { label: 'PO status' }, position: { x: 250, y: 200 }, style: getNodeStyle(isNodeCompleted('po')) },
        { id: 'artwork', data: { label: 'Artwork' }, position: { x: 450, y: 200 }, style: getNodeStyle(isNodeCompleted('artwork')) },
        { id: 'invoice', data: { label: 'Invoice details' }, position: { x: 250, y: 400 }, style: getNodeStyle(isNodeCompleted('invoice')) },
      );
      staticEdges.push(
    ...
    if (pipelineData.invoice?.invoiceNumber) {
      ...
      staticEdges.push({ id: 'e-invoice-payment', source: 'invoice', target: 'payment', markerEnd: 'arrowclosed' });
    }
    ...
    if (pipelineData.artwork?.confirmed && pipelineData.spaces.length > 0) {
      pipelineData.spaces.forEach((space, index) => {
        ...
        dynamicNodes.push(
          { id: inventoryId, data: { label: space.spaceName }, position: { x: 650, y: 100 + index * 200 }, style: getNodeStyle(true) }, // always completed
          { id: printId, data: { label: 'Printing status' }, position: { x: 850, y: 100 + index * 200 }, style: getNodeStyle(isNodeCompleted(printId, space)) },
          { id: mountId, data: { label: 'Mounting status' }, position: { x: 1050, y: 100 + index * 200 }, style: getNodeStyle(isNodeCompleted(mountId, space)) },
        );
  ...
  }, [pipelineData, spaces, fitView, setNodes, setEdges]);
